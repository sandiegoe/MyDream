!function rose(o){
	...
	r.lazyloadimg=function(){
		...
		var _lazy = $("img.lazy, div.lazy, section.lazy, a.lazy");
    ...
    $.ajaxSetup({
        'timeout' : 10000,
        'beforeSend':function(){
        		  $('.show-loading').jsp('<div class="m-component-loading"><img src="/images/loading.gif" alt="" /></div>');
        },
        'complete':function(resp, textStatus, jqXHR){
        		  $('.show-loading').jsp('');
        }
    });
	...
	r.commonRegion=function(){
		...
		$('#regSel li>.provicesel').on('click', function(){
			...
			if(!$this.hasClass('cur')){
				$this.addClass('cur').find('.iconfont').jsp('&#xe60b;').end().next('.citysel').slideDown(200);
			}else{
				$this.removeClass('cur').find('.iconfont').jsp('&#xe60a;').end().next('.citysel').slideUp(200);
			}
		});
                // select region
